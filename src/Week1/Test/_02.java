...
public class _02 {
    ...
    private static boolean searchBoard(int[][] board, int i, int j) {
        ...
        if(board[i][j] != 0){
            ...
            // 가로 탐색 - 가로 인덱스가 14보다 작거나 같아야지 끝까지 탐색가능 && 이전 오목알이 다른지 체크
            ...
            // 세로 탐색 - 세로 인덱스가 14보다 작거나 같아야지 끝까지 탐색 가능 && 이전 오목알이 다른지 체크
            if( (i <= 14 && startColor == board[i+1][j]) && !(i > 0 && startColor == board[i-1][j]) ){
                // 1,2번째 오목알은 같으므로 3,4,5번째 오목알 탐색
                for (int k = 1; k <= 3; k++) {
                    // 탐색중 같지 않으면 반목문 종료
            ...
            // 대각선 탐색 - 가로, 세로 인덱스가 14보다 작거나 같아야지 끝까지 탐색 가능 && 이전 오목알이 다른지 체크
            if( (i <= 14 && j <= 14 && startColor == board[i+1][j+1]) && !(i > 0 && j > 0 && startColor == board[i-1][j-1])){
                // 1,2번째 오목알은 같으므로 3,4,5번째 오목알 탐색
                for (int k = 1; k <= 3; k++) {
                    // 탐색중 같지 않으면 반목문 종료
            ...
            // 역대각선 탐색 - 세로 인덱스는 14보다 작거나 같고 가로 인덱스는 4보다 크거나 같아야 끝까지 탐색 가능 && 이전 오목알이 다른지 체크
            if( (i <= 14 && j >= 4 && startColor == board[i+1][j-1]) && !(i > 0 && j < 18 && startColor == board[i-1][j+1])){
                // 1,2번째 오목알은 같으므로 3,4,5번째 오목알 탐색
                for (int k = 1; k <= 3; k++) {
                    // 탐색중 같지 않으면 반목문 종료
                    if(startColor != board[i+1+k][j-1-k]){
